...
export const deleteOpenShiftProject = (
  ...
  const ocCommand = `oc delete project ${projectName} && oc wait --for=delete project/${projectName}`;
  return cy.exec(ocCommand, { failOnNonZeroExit: false, timeout: 600000 }).then((result) => {
    if (result.code !== 0) {
