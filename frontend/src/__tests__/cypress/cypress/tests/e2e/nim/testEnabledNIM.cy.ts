...
describe('Verify RHODS Explore Section Contains NIM card and verify card body', () => {
  it('Enable and validate NIM flow', () => {
    ...
    patchOpenShiftNSResource(
      ...
      '[{"op": "replace", "path": "/spec/dashboardConfig/disableNIMModelServing", "value": False}]',
      'redhat-ods-applications',
      'json',
    );
    // restart rollout on dashboard deployment
    restartRollout('rhods-dashboard', 'redhat-ods-applications');
    waitForRolloutToComplete('rhods-dashboard', 'redhat-ods-applications', 500000);
